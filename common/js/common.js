var u = 'url(../common/images/common.png)';
$(window).resize(function(){
	$('#section').setsideBarHeigh(111);
});
$(document).ready(function(){
	
	/*显示/隐藏下拉菜单*/
	$('#sysInfo').showList([{'name':'商家信息','url':'javascript:;'},{'name':'帐号管理','url':'javascript:;'},{'name':'第三方接入','url':'javascript:;'}]);
	...
	},
	{
		...
		'list':[{
			'name':'电商1',
			'links':'ec1',
			'list':[{'name':'优惠券1','bg':u+' 0px -165px','href':'#'},{'name':'优惠券3','bg':u+' 0px -165px','href':'#'}]
			}
		]
	},
	{
...
$.fn.extend({
	sideBar:function(_data){
		if(_data.length >0){
			...
			$('.second_lev li').dropDown('.third_lev');
			$('#section').setsideBarHeigh(111);
		}	
	},
	...
	setsideBarHeigh:function(val){
		var h = $(document).outerHeight()-val;
